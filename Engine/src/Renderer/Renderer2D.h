...
namespace Karem {
	...
	public:
		...
		static void SubmitQuad(const glm::mat4& transform, const std::shared_ptr<SubTexture2D>& subTexture, float texIndex = 1.0f, const glm::vec4& color = glm::vec4(1.0f));

		static void SubmitSubTexturedQuad(const glm::vec4& pos, const glm::vec2& size, const std::shared_ptr<SubTexture2D>& subTexture, float texIndex = 1.0f, const glm::vec4& color = glm::vec4(1.0f));
	...
	private:
		...
		static Renderer2DMeshes* s_Meshes;
	};
